...
public class ItemSlot : MonoBehaviour, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler
{
    ...
    {
        Debug.Log($"[ItemSlot] ━━━ OnBeginDrag 호출됨! ━━━");
        Debug.Log($"[ItemSlot] itemData: {itemData?.itemName ?? "null"}");
        Debug.Log($"[ItemSlot] slotType: {slotType}");
        Debug.Log($"[ItemSlot] ItemDragHandler.Instance: {ItemDragHandler.Instance != null}");
        Debug.Log($"[ItemSlot] eventData.button: {eventData.button}");

        // 좌클릭이 아니면 드래그 안함
        if (eventData.button != PointerEventData.InputButton.Left)
        {
            Debug.LogWarning("[ItemSlot] ⚠️ 좌클릭이 아니므로 드래그 취소");
            return;
        }

        if (itemData == null)
        {
            Debug.LogWarning("[ItemSlot] ⚠️ itemData가 null이므로 드래그 취소");
            return;
        }

        if (slotType == SlotType.Shop)
        {
            Debug.LogWarning("[ItemSlot] ⚠️ 상점 아이템은 드래그 불가");
            return;
        }
        ...
        {
            Debug.LogError("[ItemSlot] ❌ ItemDragHandler.Instance가 null입니다!");
        }
    }
    ...
    {
        // ItemDragHandler의 Update에서 처리하므로 여기서는 빈 상태로 둠
    }
    ...
    {
        Debug.Log("[ItemSlot] ━━━ OnEndDrag 호출됨! ━━━");
